...
if __name__=="__main__":
    ...
    paper_widths_mm = widths * (card_dim[1] / measurements["card_w"])
    paper_heights_mm = heights * (card_dim[0] / measurements["card_h"])
    
    for n in Ns:
        # Average pixel measurements
        width_slice_mm = paper_widths_mm[:n]
                
        # Apply calibration
        width_means.append(width_slice_mm.mean())
        # Sample standard deviation (of individual measurements)
        width_std = (width_slice_mm).std(ddof=1)
        height_std = (width_slice_mm).std(ddof=1)
    ...
    #axes[1, 1].axhline(paper_size[1], color='b', linestyle='--', alpha=0.5, label='GT W')
    if INCLUDE_HEIGHT:
